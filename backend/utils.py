...
from backend.routes.admin.admin_db import (
    create_team,
)  # Move this import from database.py
...
def add_teams_from_json(session, teams_json_path):
    try:
        with open(teams_json_path, "r") as file:
            ...
            teams_list = data.get(
                "teams", []
            )  # Use .get to safely handle missing 'teams' key
        ...
        for team_data in teams_list:
            ...
            create_result = create_team(
                session=session,
                name=team_data["name"],
                password=team_data["password"],
                school=team_data.get("school", None),
            )

            # Check for errors in team creation
            if create_result.get("status") == "failed":
                raise ValueError(
                    f"Failed to create team '{team_data['name']}': {create_result['message']}"
                )

    except FileNotFoundError:
        raise FileNotFoundError(
            f"Error: 'teams_json_path' not found at: {teams_json_path}"
        )
