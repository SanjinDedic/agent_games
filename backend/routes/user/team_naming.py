...
def create_school_team(
    ...
    Race-safe: on IntegrityError from the unique-name constraint, roll back
    and re-query — a competing transaction's commit becomes visible after
    rollback under READ COMMITTED, so the next candidate naturally skips
    past it. Retries up to MAX_COLLISION_RETRIES times.
    """
