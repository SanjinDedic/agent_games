...
def test_add_teams_from_json(db_session, tmpdir, mocker):
    ...
    mock_create_team = mocker.patch(
        "utils.create_team", return_value={"status": "success"}
    )

    add_teams_from_json(db_session, str(teams_json_path))

    assert mock_create_team.call_count == 2
    mock_create_team.assert_any_call(
        session=db_session, name="TestTeam1", password="password1", school="TestSchool1"
    )
    mock_create_team.assert_any_call(
        session=db_session, name="TestTeam2", password="password2", school="TestSchool2"
    )
...
def test_add_teams_from_json_file_not_found(db_session):
    ...
    assert f"Error: 'teams_json_path' not found at: {teams_json_path}" in str(
        excinfo.value
    )
...
def test_add_teams_from_json_create_team_failed(db_session, tmpdir, mocker):
    ...
    mocker.patch(
        "utils.create_team",
        return_value={"status": "failed", "message": "Failed to create team"},
    )

    with pytest.raises(ValueError) as excinfo:
        add_teams_from_json(db_session, str(teams_json_path))
    assert "Failed to create team 'TestTeam1': Failed to create team" in str(
        excinfo.value
    )
