...
from backend.database.db_models import AgentAPIKey, League, Admin, Team, TeamType
...
def test_setup_claims_an_unclaimed_deployment(client, db_session: Session):
    """The seeded test database has no admin, so setup succeeds and logs in."""
    ...
    admin = db_session.exec(select(Admin)).one()
    assert admin.username == "the_teacher"
    assert admin.verify_password("a-good-password")


def test_setup_is_refused_once_an_admin_exists(client):
...
def test_config_reports_setup_required(client):
    ...
    client.post("/auth/setup", json={"name": "admin", "password": "password123"})
...
def test_login_as_admin(client, db_session: Session):
    admin = Admin(username="site_admin", password_hash=TEST_PASSWORD_HASHES["test_password"])
    db_session.add(admin)
    db_session.commit()

    response = client.post(
        "/auth/login", json={"name": "site_admin", "password": "test_password"}
    )
    assert response.status_code == 200
    body = response.json()
    assert body["role"] == "admin"
...
def test_login_failures_are_indistinguishable(client, db_session: Session):
    ...
    reveals whether a name exists — and whether it is the admin's."""
    admin = Admin(username="site_admin", password_hash=TEST_PASSWORD_HASHES["test_password"])
    db_session.add(admin)
    db_session.commit()

    unknown = client.post("/auth/login", json={"name": "nobody", "password": "whatever"})
    wrong_admin_pw = client.post("/auth/login", json={"name": "site_admin", "password": "nope"})
    wrong_team_pw = client.post("/auth/login", json={"name": "TeamA", "password": "nope"})

    for response in (unknown, wrong_admin_pw, wrong_team_pw):
...
def test_expired_token_is_rejected(client):
    ...
    response = client.get("/admin/get-all-teams", headers={"Authorization": f"Bearer {expired}"})
    assert response.status_code == 401


def test_malformed_token_is_rejected(client):
    response = client.get(
        "/admin/get-all-teams", headers={"Authorization": "Bearer not.a.jwt"}
    )
    assert response.status_code == 401


def test_unknown_role_is_rejected(client):
    """get_current_user only accepts admin, student and ai_agent."""
    token = create_access_token(
        data={"sub": "whoever", "role": "institution"},
        expires_delta=timedelta(minutes=30),
    )
    response = client.get("/admin/get-all-teams", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 401
...
def test_team_token_cannot_reach_admin_routes(client, team_headers):
    response = client.get("/admin/get-all-teams", headers=team_headers)
    assert response.status_code == 403


def test_admin_token_cannot_reach_team_routes(client, admin_headers):
    """require_team refuses a token with no team_id."""
    response = client.get("/user/get-team-submissions", headers=admin_headers)
    assert response.status_code == 403
