...
class RandomPlayer(Player):
    ...
    strategy = "Drops its piece into a random valid position every turn."
...
class LeftStackPlayer(Player):
    ...
    strategy = (
        "Always plays in the leftmost column that still has space, "
        "filling the board left to right."
    )
...
class MrCenter(Player):
    ...
    strategy = (
        "Prefers the centre column and works outwards (4, 3, 5, 2, 6, 1, 7) — "
        "centre squares touch the most winning lines."
    )
...
class RandomUntilWin(Player):
    strategy = (
        "Plays randomly, but always takes a winning move the moment "
        "one is available."
    )
...
class BlockUntilWin(Player):
    ...
    strategy = (
        "Takes a winning move if one exists, otherwise blocks the opponent's "
        "winning move, otherwise plays randomly."
    )
