...
function AdminDemoUsers() {
    ...
    const handleDeleteAll = async () => {
      if (
        !window.confirm(
          "Are you sure you want to delete ALL demo users? This action cannot be undone."
        )
      ) {
        return;
      }
    ...
    const handleDeleteUser = async (teamId, teamName) => {
      if (
        !window.confirm(
          `Are you sure you want to delete demo user ${teamName}?`
        )
      ) {
        return;
      }
      ...
      try {
        ...
        if (data.status === "success") {
          ...
          setDemoUsers(
            demoUsers.filter((user) => user.demo_team_id !== teamId)
          );
        } else {
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            {isLoading ? (
              <div className="flex justify-center items-center h-32">
                <div className="text-lg text-ui-dark">
                  Loading demo users...
                </div>
              </div>
            ) : demoUsers.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
                {demoUsers.map((user) => (
                  <DemoUserCard
                    ...
                    user={user}
                    onDelete={() =>
                      handleDeleteUser(user.demo_team_id, user.demo_team_name)
                    }
                  />
                ))}
              </div>
