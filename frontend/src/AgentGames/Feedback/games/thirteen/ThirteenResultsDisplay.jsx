    ...
    useEffect(() => {
        if (data && data.total_points) {
            const resultData = Object.keys(data.total_points).map((team) => ({
                ...
                avgFinish: data.table?.avg_finish?.[team],
                bombs: data.table?.bombs_played?.[team] ?? 0,
            }));
                ...
                <div className="w-full overflow-x-auto bg-white rounded-lg shadow-lg border border-ui-light">
                  ...
                  `}
                                >
                                    ...
                                    <td className="p-4 border-b border-ui-light/30 font-medium text-notice-orange">
                                        {result.bombs?.toLocaleString() || 0}
                                    </td>
                    ...
                    <div className="px-4 py-2 text-xs text-ui">
                        ...
                        (1st = 4, 2nd = 2, 3rd = 1, 4th = 0). Avg Finish is the mean placing (1 = best, lower is
                        better); Games Won counts first-place finishes.
                    </div>
                </div>
            )}
        </div>
...
export default ThirteenResultsDisplay;
