...
const ArenaChampionsFeedback = ({ feedback }) => {
    ...
    useEffect(() => {
        if (feedback?.battles) {
            const battles = selectedPlayer === 'all'
                ? feedback.battles
                : feedback.battles.filter(battle =>
                    battle.player1 === selectedPlayer || battle.player2 === selectedPlayer
                );
    ...
    if (filteredBattles.length === 0) {
        return <div className="text-ui-dark">No battles found for the selected filters</div>;
    }

    const currentBattleData = filteredBattles[selectedBattle] || filteredBattles[0];
    ...
    const hasPreview = currentBattleData.player1_has_preview;
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-lg">
            <div className="mb-6 space-y-4">
                ...
                <div>
                    <label className="block text-ui-dark font-medium mb-2">Select Battle</label>
                    <select
                    ...
                    >
                        {filteredBattles.map((battle, index) => (
                            <option key={index} value={index}>
                                {battle.player1} vs {battle.player2} {battle.player1_has_preview ? '(Preview)' : '(No Preview)'}
                            </option>
                        ))}
                    </select>
                </div>
            ...
            <div className="mb-6 text-center">
                ...
                <div className="flex items-center justify-center gap-4 text-sm text-ui mb-2">
                    <span className={`px-2 py-1 rounded ${hasPreview ? 'bg-success/20 text-success' : 'bg-ui-lighter text-ui'}`}>
            ...
            <div className="flex justify-center items-center gap-6 mt-6">
                ...
                <span className="text-lg text-ui-dark">
                    {battleComplete ? 'Battle Complete' : `Turn ${currentTurnIndex + 1} of ${turns.length}`}
                </span>
                <button
                    onClick={handleNext}
