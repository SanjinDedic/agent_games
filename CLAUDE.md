...
# Run with coverage
docker compose -f docker-compose.yml -f docker-compose.test.yml run --rm test-runner pytest --cov=backend --cov-report=term backend/tests/ backend/fallback_lambda/tests/
```

The fallback-Lambda suite lives in `backend/fallback_lambda/tests/` (its own `testpaths` entry) so deleting that folder deletes its tests atomically.
...
**Server-side agent validation runs on AWS Lambda, not Celery** (Celery is fully removed). It lives self-contained in `backend/validation_lambda/` — unlike the exercise fallback below, this is PERMANENT infra: hint requests always need a fresh server-run envelope. In prod the API invokes the Lambda `agent-games-validation` (`VALIDATION_LAMBDA_FUNCTION` env in `config/deploy.yml`, deployed via the folder's `deploy.sh` — the zip snapshots `backend/games/` plus a generated `config.py` stub, so redeploy after adding a game); with the env unset (dev, tests, emergency rollback) the same executor runs as a scrubbed-env local subprocess (`local_run.py` additionally stubs `backend.config` in `sys.modules` so `load_dotenv` can't re-inject secrets). The AST safety check still runs in the API process first (`backend/routes/user/code_validation.py`); the sandbox gives each run a fresh forked child with 5s soft / 6s hard time limits (plain constants in `executor.py`, not env-tunable) and a process-group SIGKILL. The 7-key envelope and its message prefixes are contracts: `hint_context.py` prefix-matches them and `test_validation_harness_parity.py` pins them against the browser harness.

**Tutorial exercises and lesson snippets are Pyodide-first in the browser** (`frontend/src/pyodide/exerciseRunnerClient.js` + `exercise_harness.py`, parity pinned by `backend/fallback_lambda/tests/test_exercise_harness_parity.py`). **The server never executes exercise or snippet code.** The fallback is browser→Lambda direct, student/team pages only: when Pyodide can't run, the browser calls the `agent-games-exercise-fallback` Function URL (`VITE_EXERCISE_LAMBDA_URL` in `frontend/.env.production`; `backend/fallback_lambda/`, deployed via its `deploy.sh`, python3.14/arm64/1769MB, no-egress VPC) with a 10s exec token from `POST /auth/execution-token` (`EXEC_JWT_SECRET` must match API and Lambda). Student exercise fallbacks persist via `POST /tutorial/submit-exercise-result` tagged `execution_source="pyodide_fallback"` (rows stamped `lambda_direct`); snippet fallbacks fire `POST /lesson/snippet-fallback-beacon`; both are counted at `GET /diagnostics/pyodide-fallbacks` (snippet reasons carry a `snippet:` prefix). `VITE_PYODIDE_EXERCISES=false` builds also go browser→Lambda (reason `pyodide-disabled`). Admin/institution surfaces (exercise preview, admin editor Run, lesson preview) are Pyodide-only — no Lambda, they error when Pyodide can't run. Student code executes with NO AST safety gate — the Lambda is the sandbox: no secrets/DB/S3, a fresh forked child per run, 0.5s soft / 1.5s hard time limits (plain constants, not env-tunable), process-group SIGKILL. `executor.py`/`handler.py`/`exec_token.py` must stay stdlib-only (the zip ships nothing else); the folder owns its tests (`backend/fallback_lambda/tests/`) and is designed to be deleted once fallback telemetry shows a sustained zero.

**Agent validation is Pyodide-first in the browser** for every game (`frontend/src/pyodide/validationRunnerClient.js` + `validation.worker.js` + `validation_harness.py`, semantics pinned by `backend/tests/unit/test_validation_harness_parity.py`). The submission page's runner is health-gated per game: on load it runs a FULL validation of the game's own `starter_code` read off the engine tree (nothing sent to the server, nothing saved) — only a passing probe enables the local path, and `test_starter_code_validates_for_every_game` keeps that probe green in CI. Local results (success AND failure) persist via `POST /user/submit-agent-result`, which re-runs the AST gate (stored code is later executed in teachers' browsers by the league simulation runner), shares the 5/min `SubmissionMetadata` budget, and computes the ranking server-side. The server path above only sees: hint requests (`generate_hint=true` always goes to the server), `VITE_PYODIDE_VALIDATION=false` builds, and fallback traffic tagged `execution_source="pyodide_fallback"` (counted at `GET /diagnostics/pyodide-fallbacks` with a `validation:` reason prefix).
...
- `validation_lambda/` — Server-side agent validation: `executor.py` (run body + fork isolation, parity-pinned), `handler.py`/`local_run.py` (Lambda / local subprocess entry points), `client.py` (the async API the routes call), `deploy.sh`
...
- Execution-path tests run the real local-subprocess executor (`VALIDATION_LAMBDA_FUNCTION`/`EXERCISE_LAMBDA_FUNCTION` are unset in tests) — no workers, no broker; the isolation tests fork real children and monkeypatch the executor's limit constants to stay fast
