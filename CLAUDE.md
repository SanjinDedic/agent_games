...
**Tutorial exercises and lesson snippets are Pyodide-first in the browser** (`frontend/src/pyodide/exerciseRunnerClient.js` + `exercise_harness.py`, parity pinned by `backend/tests/unit/test_exercise_harness_parity.py`). The server fallback is NOT a Celery worker: it lives self-contained in `backend/fallback_lambda/` and serves only fallback traffic (tagged `execution_source="pyodide_fallback"`, counted at `GET /diagnostics/pyodide-fallbacks` — snippet reasons carry a `snippet:` prefix) and `VITE_PYODIDE_EXERCISES=false` builds. In prod the API invokes the AWS Lambda `agent-games-exercise-fallback` (`EXERCISE_LAMBDA_FUNCTION` env in `config/deploy.yml`, deployed via the folder's `deploy.sh`, python3.14/arm64/1769MB); with the env unset (dev, tests, emergency rollback) the same executor runs as a scrubbed-env local subprocess. Student code executes with NO AST safety gate — the Lambda (or subprocess) is the sandbox: no secrets/DB/S3, a fresh forked child per run, 0.5s soft / 1.5s hard time limits (plain constants, not env-tunable), process-group SIGKILL. `executor.py`/`handler.py`/`local_run.py` must stay stdlib-only (the zip ships nothing else); the folder is designed to be deleted once fallback telemetry shows a sustained zero.
