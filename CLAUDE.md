...
- **worker-exercises**: Standalone slim Celery app (`backend/exercise_worker/tasks.py`, its own image: python:alpine + celery + that one file) consuming the `exercises` queue at concurrency 2. Tutorial exercises (`exercises.run`) and lesson snippets (`exercises.run_snippet`) run here with NO AST safety gate — the container is the sandbox (no secrets/DB/S3, ~96MB RAM, 50 pids) — under a 0.5s soft / 1.5s hard time limit (plain constants, not env-tunable). The API enqueues by task name via `send_task`; the module must stay stdlib+celery only since the slim image ships nothing else. Both paths are Pyodide-first in the browser (`frontend/src/pyodide/exerciseRunnerClient.js` + `exercise_harness.py`, parity pinned by `backend/tests/unit/test_exercise_harness_parity.py`); this worker only sees fallback traffic (tagged `execution_source="pyodide_fallback"`, counted at `GET /diagnostics/pyodide-fallbacks` — snippet reasons carry a `snippet:` prefix) and `VITE_PYODIDE_EXERCISES=false` builds
