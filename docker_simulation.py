...
def run_docker_simulation(num_simulations, league_name, league_game, league_folder, custom_rewards):
    ...
    print("Raw Docker Output:", docker_results.stdout)
...
def run_setup_script():
    try:
        result = subprocess.run(["bash", "./setup.sh"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True)
        print(result.stdout)
    except subprocess.CalledProcessError as e:
        print(f"Error occurred while running setup.sh: {e.stderr}")
        return False
    return True